...
export function Sidebar() {
  ...
  return (
    <aside className={cn(
      // Sticky: stays in the flex flow, not an overlay
      "sticky top-0 h-screen shrink-0 flex flex-col transition-all duration-200",
      "bg-white border-r border-gray-200 dark:bg-gray-900 dark:border-gray-700",
      collapsed ? "w-14" : "w-56"
    )}>
      {/* Logo + collapse toggle */}
      <div className="flex items-center gap-2 px-3 h-14 shrink-0 border-b border-gray-100 dark:border-gray-700">
        ...
        {!collapsed && (
          <span className="font-semibold text-gray-900 dark:text-gray-100 text-sm truncate flex-1">
            SquareLabs
          </span>
        )}
        <button
          onClick={() => setCollapsed(c => !c)}
          className="ml-auto text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 shrink-0 p-1 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        {visibleNav.map(group => {
          ...
          const groupActive = group.children.some(c =>
            pathname === c.href.split("?")[0] || pathname.startsWith(c.href.split("?")[0] + "/")
          );
          return (
            <div key={group.label}>
              <button
                ...
                className={cn(
                  "sidebar-item w-full",
                  groupActive && "text-accent-600 bg-accent-50 dark:text-blue-400 dark:bg-blue-900/40"
                )}
              >
      ...
      <div className="border-t border-gray-100 dark:border-gray-700 px-2 py-2 space-y-0.5">
        ...
        <button onClick={toggle} title={collapsed ? (theme === "dark" ? "Light mode" : "Dark mode") : undefined}
        ...
        {/* User card */}
        <div className={cn("flex items-center gap-2 px-2 py-2 mt-1 rounded-lg", collapsed && "justify-center")}>
          <div className="w-7 h-7 rounded-full bg-brand-100 dark:bg-brand-900 flex items-center justify-center shrink-0">
            <span className="text-brand-700 dark:text-brand-300 font-semibold text-xs">{initials}</span>
          </div>
          {!collapsed && (
